...
# vqvaemultiheadtransformer_model = VQVAEGANMultiHeadTransformer()
# # vqvaemultiheadtransformer_model = VQVAEGANMultiHeadTransformer(ex_multi_scale_num=0, enable_mid=False, ch_mult=(1, 2), attn_resolutions=(), num_res_blocks=1, z_channels=64)
# print("Number of parameters in VQVAEGANMultiHeadTransformer:", sum([p.numel() for p in vqvaemultiheadtransformer_model.parameters()])/1e6, "M")
# model_weights = torch.load("last.ckpt")
# state_dict = model_weights['state_dict']
# new_state_dict = {}
# for key in state_dict.keys():
#     # Skip the 'encoder.norm_out', encoder.conv_out, decoder.conv_in and 'quant_conv'
#     # if 'encoder.norm_out' in key or 'encoder.conv_out' in key or 'decoder.conv_in' in key or 'quant_conv' in key:
#     #     continue
#     # Remove the 'vqvae.' prefix from keys
#     new_key = key.replace('vqvae.', '')
#     new_state_dict[new_key] = state_dict[key]

# missing_keys, unexpected_keys = vqvaemultiheadtransformer_model.load_state_dict(new_state_dict, strict=False)
# print(f"Missing keys: {missing_keys}")
# # print(f"Unexpected keys: {unexpected_keys}")
# missing_params = sum(vqvaemultiheadtransformer_model.state_dict()[key].numel() for key in missing_keys)
# print(f"Number of missing parameters (in Million): {missing_params / 1e6} M")

# # # Create a random tensor with the shape of the input tensor and pass it through the model
# input_tensor = torch.randn((1, 3, 512, 512))
# print("Min: ", input_tensor.min().item(), "Max: ", input_tensor.max().item())
# if torch.cuda.is_available():
#     vqvaemultiheadtransformer_model = vqvaemultiheadtransformer_model.cuda()
#     input_tensor = input_tensor.cuda()
# else:
#     raise Exception("CUDA not available")
# output_tensor, _, _, _ = vqvaemultiheadtransformer_model(input_tensor)
# print("Output tensor shape:", output_tensor.shape)

# # print(vqvaemultiheadtransformer_model)

# from PIL import Image
# # Read 'input.png' as RGB
...
# print("Min: ", input_tensor.min().item(), "Max: ", input_tensor.max().item())
# # Normalize the image tensor using the mean and std of the ImageNet dataset
# input_tensor = input_tensor / 255.0
# input_tensor = (input_tensor - torch.tensor([0.5, 0.5, 0.5])) / torch.tensor([0.5, 0.5, 0.5])
# print("Min: ", input_tensor.min().item(), "Max: ", input_tensor.max().item())
# input_tensor = input_tensor.unsqueeze(0).permute(0, 3, 1, 2).float()
...
# output_tensor, _, _, _ = vqvaemultiheadtransformer_model(input_tensor)
# print("Output tensor shape:", output_tensor.shape)
# output_tensor = output_tensor.squeeze().detach().cpu().numpy().transpose(1, 2, 0)
# output_image = (output_tensor * np.array([0.5, 0.5, 0.5]) + np.array([0.5, 0.5, 0.5]))
