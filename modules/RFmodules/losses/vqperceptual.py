...
from modules.RFmodules.discriminator.model import (NLayerDiscriminator,
                                                       weights_init)
from modules.RFmodules.losses.lpips import LPIPS
from modules.RFmodules.vqvae.arcface_arch import ResNetArcFace
from modules.RFmodules.vqvae.facial_component_discriminator import \
...
class VQLPIPSWithDiscriminatorWithCompWithIdentity(nn.Module):
    def __init__(self, disc_start=0, codebook_weight=1.0, pixelloss_weight=1.0,
                 ...
                 identity_weight=0.0, lpips_style_weight=0.0,
    ...
    def forward(self, codebook_loss, gts, reconstructions, global_step, optimizer_idx, last_layer=None, split="train"):
        
        # Generator update
        if optimizer_idx == 0:
            # print("Great! The Generator is updating!")
            ...
            if self.identity_weight > 0. and self.discriminator_iter_start < global_step:
                # print(f"Identity loss is calculated. Identity weight: {self.identity_weight}, global step: {global_step}")
                self.identity.eval()
            ...
            # else:
            #     print(f"Identity loss is not calculated. Identity weight: {self.identity_weight}, global step: {global_step}")
        ...
        if optimizer_idx == 1:
            # print("Great! The Discriminator is updating!")
