    ...
    for i in range(0, len(trans_z), stride):
        ...
        if np.max(window_z) - np.min(window_z) > threshold_z:
            if np.max(window_x) - np.min(window_x) < threshold_x and np.max(window_y) - np.min(window_y) < threshold_y and np.max(window_euler_x) - np.min(window_euler_x) < threshold_euler_x and np.max(window_euler_y) - np.min(window_euler_y) < threshold_euler_y and np.max(window_euler_z) - np.min(window_euler_z) < threshold_euler_z:
                # print(f"Removing wobbling between time steps {i/25:.2f} and {(i+window_size)/25:.2f} seconds")
                smoothed_z[max(0,i-window_size//2):min(i+window_size+window_size//2,len(trans_z))] = medfilt(trans_z[max(0,i-window_size//2):min(i+window_size+window_size//2,len(trans_z))] , kernel_size=window_size)
                smoothed_z[i:i+window_size] = medfilt(smoothed_z[i:i+window_size], kernel_size=(window_size // 2) | 1)
                smoothed_z[max(0,i-window_size//2):min(i+window_size+window_size//2,len(trans_z))] = savgol_filter(smoothed_z[max(0,i-window_size//2):min(i+window_size+window_size//2,len(trans_z))], window_size, (window_size // 4) | 1)
